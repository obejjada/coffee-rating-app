""" Test Module used to hold the unit tests that will verify the methods
...
class TestDatabaseDeletion():
    ...
    def setup_method(self):
        ...
        if not os.path.exists(CURRENT_PATH + r'\Database'):
            ...
            connection = sqlite3.connect(CURRENT_PATH + r'\Database\coffee'
            ...
            Rating real)""")
            connection.commit()
            test_inputs = [("05/09/2022", "Bristol Coffe Shop", "Americano' \
                            'Black", 4.5),
                           ("01/05/2008", "Southampton Coffe Shop",
                            "Cappuccino", 2.0),
                           ("29/04/1957", "London Coffe Shop", "Flat White",
                            0.5)]
            cursor.executemany("INSERT INTO Rating_Table VALUES (?,?,?,?)",
                               test_inputs)
            connection.commit()
            connection.close()
    ...
    def test_delete_entry(self):
        """ Method to verify if a specified record in the application database
        has been deleted"""
        ...
        connection = sqlite3.connect(CURRENT_PATH + r'\Database\coffee-rating'
                                     r'-app-database.db')
        cursor = connection.cursor()
        database_contents = cursor.execute("SELECT * FROM Rating_Table"
                                           ).fetchall()
        connection.close()
