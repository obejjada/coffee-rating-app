...
class TestDatabaseCreation():
    ...
    def test_create_table(self):
        """ Method to verify a table is created in the database with the
        correct title and column headings
        Title: Rating Table
        Column 1: Date (Text)
    ...
    def test_add_record(self):
        """Method to verify that a record has been correctly added
        to the database"""
        ...
        connection.commit()
        connection.close()
        self.databasecreation.add_record(TEST_DATABASE_PATH, "05/09/2022",
                                         "Bristol Coffee Shop",
                                         "Americano Black", 4.5)
        connection = sqlite3.connect(CURRENT_PATH + r'\Database\coffee-rating'
                                     r'-app-database.db')
